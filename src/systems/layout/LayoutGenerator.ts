...
export class LayoutGenerator {
    ...
    private readonly OFFICE_ELEMENTS: OfficeElement[] = [
        // Rooms
        { name: 'Cubicle', width: 120, height: 100, color: 0x001835, borderColor: 0x004657, category: 'room' },
        { name: 'Private Office', width: 150, height: 120, color: 0x001b26, borderColor: 0x004559, category: 'room' },
        { name: 'Conference Room', width: 200, height: 150, color: 0x001a27, borderColor: 0x00455e, category: 'room' },
        { name: 'Break Room', width: 180, height: 130, color: 0x001a2b, borderColor: 0x007689, category: 'room' },
        
        // Furniture
        { name: 'Desk', width: 80, height: 40, color: 0x7b111f, borderColor: 0x370016, category: 'furniture' },
        { name: 'Filing Cabinet', width: 40, height: 60, color: 0x69292f, borderColor: 0x300621, category: 'furniture' },
        { name: 'Bookshelf', width: 60, height: 100, color: 0x543640, borderColor: 0x1f122d, category: 'furniture' },
        { name: 'Meeting Table', width: 120, height: 60, color: 0xbf3825, borderColor: 0x001b26, category: 'furniture' },
        
        // Equipment
        { name: 'Printer', width: 50, height: 50, color: 0xa74f3c, borderColor: 0x001a27, category: 'equipment' },
        { name: 'Computer', width: 40, height: 30, color: 0x8c5f53, borderColor: 0x001a2b, category: 'equipment' },
        { name: 'Projector', width: 30, height: 20, color: 0xf3714a, borderColor: 0x373f52, category: 'equipment' },
        { name: 'Coffee Machine', width: 40, height: 60, color: 0xdc8060, borderColor: 0x004559, category: 'equipment' },
        
        // Hazards
        { name: 'Spilled Coffee', width: 40, height: 40, color: 0xc48c76, borderColor: 0x00455e, category: 'hazard' },
        { name: 'Loose Wires', width: 30, height: 30, color: 0xffbc9f, borderColor: 0x007689, category: 'hazard' },
        { name: 'Paper Stack', width: 30, height: 40, color: 0x8dd8ed, borderColor: 0x347482, category: 'hazard' },
        { name: 'Broken Printer', width: 50, height: 50, color: 0xb2d2d7, borderColor: 0xa7968d, category: 'hazard' }
    ];
    ...
    generateNewLayout(): void {
        ...
        this.currentLayout.push({ rectangle: room, text: roomText, element: { name: 'Main Office', width: this.roomConfig.width, height: this.roomConfig.height, color: 0x001835, borderColor: 0x004657, category: 'room' } });
        ...
        for (let i = 0; i < elementCount; i++) {
            ...
            if (position) {
                ...
                const text = this.scene.add.text(
                    position.x,
                    position.y - element.height/2 - 10,
                    element.name,
                    {
                        fontSize: '12px',
                        color: '#ffffff',
    ...
    checkCollision(x: number, y: number, size: number): boolean {
        ...
        for (let i = 1; i < this.currentLayout.length; i++) {
            ...
            if (this.isOverlapping(x, y, size, size, obstacle)) {
                return true;
            }
        }
